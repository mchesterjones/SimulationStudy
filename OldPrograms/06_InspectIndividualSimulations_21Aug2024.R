...
#################################################################################
  ## Brier Score
################################################################################
  Brier_individuals <- (simulation_results[["iterations"]][[4]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Prediction_Model"]] - 
                          simulation_results[["iterations"]][[4]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Y"]])^2 ## Predicted risks - true outcome
  Brier <- mean(Brier_individuals) # Average of these sqaured errors
  Brier_var <- var(Brier_individuals)/length(simulation_results[["iterations"]][[4]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Prediction_Model"]]) # Variance of the individual Brier Scores 
  # Calculate the baseline Brier score
  p_baseline <- mean( simulation_results[["iterations"]][[4]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Y"]]) # Prevalence of the outcome
  Brier_baseline <- mean((p_baseline -  simulation_results[["iterations"]][[4]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Y"]])^2)
  # Scale the Brier score
  Brier_scaled <- 1 - (Brier / Brier_baseline)
################################################################################
# Performance Measures ReCreate
################################################################################
Predicted_Risks_CCA <-simulation_results[["iterations"]][[88]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Prediction_Model"]]
Y_CCA <- simulation_results[["iterations"]][[88]][["preds"]][["preds_per_data_set"]][["CCA_val_data"]][["Y"]]
