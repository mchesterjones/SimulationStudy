...
### Bias
####################
bias_x_limits <- c(-0.06,0.07)
bias_x_breaks <- seq(-1,1,by=0.025)
...
## Calibration Slope
###########################
  cal_slope_x_limits=c(-1,3.5)
  cal_slope_x_breaks=seq(-1,4,by=0.5)
...
for (measure_name in names(plot_storage)) {
  ...
  patchwork_storage[[measure_name]] <- list()
    ...
    output_file <- paste0("Patchwork_MissingnessMechanism_500_", measure_name, ".pdf")
